...
def main():
    ...
    try:
        ...
        if existing_student:
            ...
            # 최신 수업 데이터로 추세 업데이트
            latest_data = system.df[system.df["student_id"] == existing_student].iloc[
                -1
            ]
        ...
        if (
            feedback
            and "오류" not in feedback
            and "피드백 생성 중 오류" not in feedback
        ):
            print("\n🤖 AI 생성 피드백")
            print("-" * 50)
            print(feedback)
...
def generate_feedback_with_context(
    system, student_id, attitude, understanding, homework, qa, progress, memo
):
    ...
    try:
            ...
            previous_records.append(
                {
                    ...
                    "feedback_보완": row.get("수업보완", "N/A"),
                    "feedback_태도": row.get("수업태도", "N/A"),
                    "feedback_comment": row.get("전체수업 Comment", "N/A"),
                }
            )
        ...
        if len(student_data) >= 2:
            prev_row = student_data.iloc[-2]  # 직전 수업
            previous_lesson = {
                "date": prev_row["date"].strftime("%Y-%m-%d"),
                "progress": prev_row.get("progress_text", "N/A"),
                "memo": prev_row.get("class_memo", "N/A"),
                "feedback_보완": prev_row.get("수업보완", "N/A"),
                "feedback_태도": prev_row.get("수업태도", "N/A"),
                "feedback_comment": prev_row.get("전체수업 Comment", "N/A"),
            }
...
- 수업내용: {previous_lesson["progress"]}
- 특이사항: {previous_lesson["memo"]}
- 이전 피드백 요약:
  * 수업보완: {previous_lesson["feedback_보완"]}
  * 수업태도: {previous_lesson["feedback_태도"]}
  * 전체코멘트: {previous_lesson["feedback_comment"]}''' if previous_lesson else '[바로 전 수업 정보 없음]'}
...
- 이전 수업 기록과 바로 전 수업의 피드백을 참고하여 연속성 있는 평가 작성
- 바로 전 수업에서 언급된 개선점이 이번 수업에서 어떻게 변화했는지 반영
- 각 섹션은 3-5문장으로 작성하고, 점수는 언급하지 말고 학생의 행동과 태도에 집중
- 전체 comment에서 파트를 나눈 것을 드러내지말고 자연스럽게 연결할 것.
- 학부모님께 전달할 수 있는 형태로 작성
"""

        # 프롬프트 내용 출력 (디버깅용)
        print("\n" + "="*60)
        print("🔍 AI에게 전달되는 프롬프트:")
        print("="*60)
        print("\n[SYSTEM MESSAGE]")
        print(system_msg)
        print("\n[USER MESSAGE]")
