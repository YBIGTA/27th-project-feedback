...
class FeedbackSystem:
    ...
    def find_student_by_name(self, student_name: str) -> str:
        ...
        partial_match = self.df[self.df['student_name'].str.contains(student_name, na=False)]
    ...
    def add_new_student(self, student_id: str, student_name: str, 
                        grade: str = "고1", subject: str = "수학") -> bool:
        """신규 학생 추가"""
        try:
            # 이미 존재하는 학생인지 확인
            ...
            # 데이터프레임에 추가
            self.df = pd.concat([self.df, pd.DataFrame([new_student_data])], 
                               ignore_index=True)
    ...
    def calculate_score_changes(self, student_id: str) -> Dict[str, Any]:
        ...
        # 점수 변화 계산
        changes = {}
        score_columns = ['attitude_score', 'understanding_score', 
                        'homework_score', 'qa_score']
    ...
    def generate_feedback(self, student_id: str) -> str:
        ...
        try:
            ...
            학부모님께 전달할 수 있는 전문적이고 따뜻한 피드백을 작성합니다.
...
- 학부모님이 학생의 학습 상황을 종합적으로 파악할 수 있도록 작성"""
    ...
    def generate_feedback_for_new_student(self, student_id: str, attitude: int, 
                                        understanding: int, homework: int, qa: int, 
                                        progress: str, memo: str) -> str:
...
- 학부모님이 학생의 학습 상황을 종합적으로 파악할 수 있도록 작성"""
    ...
    def _get_llm(self, model: str = "solar-pro-250422", 
            ...
            if "error" not in changes:
                ...
                for col, data in changes['changes'].items():
                    if "error" not in data:
                        ...
                        print(f"  {col_name}: {data['current']}점 {data['symbol']} "
